...
def getDisjointPaths(graph, source, sink):
    items = algorithms.ksp_xuemei(graph, source, sink)
    #print "Yen's algorithm:"
    #for path in items:
    #    print "Cost:%s\t%s" % (path['cost'], "->".join(path['path']))
    items = filterPathsWithCircle(items)
    #print "Filter paths with circle:"
    #for path in items:
    #    print "Cost:%s\t%s" % (path['cost'], "->".join(path['path']))
    items = filterPaths(items)
    #print "Filter paths not valid:"

    #print result
    #print source, sink, items
    #for path in items:
    #    print "Cost:%s\t%s" % (path['cost'], "->".join(path['path']))
    return items

def filterPathsWithCircle(paths):
    '''
    added by Xuemei
    '''
    result = []
    for path in paths:
        ...
        for node in path['path']:
            if node not in nodeSet:
                ...
                includeCircle = True
        if not includeCircle:
            result.append(path)
    return result

def filterPaths(paths):
    '''
    each path in paths: s, node1, node2, ..., t
    rules:
    1. keep paths with shorter distance. One path at the front is shorter than another path at the back
    2. suppose previous paths are p1,...,pi-1. For path pi, if (pi-pi&pk) == None or (pk-pi&pk) == None (for k=1,...,i-1), remove pi
    3. if (pi-pi&k) is a subset of sum(px-(px&py)) (x,j >= 1, <= i-1), remove pi
    '''
    result = []
    ...
    while True:
        ...
        for i in range(len(paths)):
            now = Set(paths[i]['path'])
            for j in range(i+1, len(paths)):
                nex = Set(paths[j]['path'])
                intersectSet = intersectSet.union(now.intersection(nex))

        for i in range(len(paths)):
            now = Set(paths[i]['path'])
            if i == 0:
                #not delete the shortest path
                continue
            if now.issubset(intersectSet):
                toRemovePath = paths[i]
                break
        if toRemovePath is not None:
            paths.remove(toRemovePath)
        else:
            break
    return paths
