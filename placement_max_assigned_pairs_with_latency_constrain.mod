...
#5.1.1
#subject to both_tasks_assigned_1 {s in 1..m, t in 1..m} : a[s,t] <= u[s,t];
#5.1.2
#subject to both_tasks_assigned_2 {s in 1..m, t in 1..m} : (sum {i in 1..n} sum {j in 1..n} (g[s,i]+g[t,j])) >= (2 * u[s,t]);
#5.1.3
#subject to both_tasks_assigned_3 {s in 1..m, t in 1..m} : (sum {i in 1..n} sum {j in 1..n} (g[s,i]+g[t,j])) <= ((1 - u[s,t]) * (2 - epsilon) + 2 * u[s,t]);
#or
#5.2
subject to both_tasks_assigned_1 {s in 1..m, t in 1..m} : 2 * a[s,t] <= (sum {i in 1..n} sum {j in 1..n} (g[s,i]+g[t,j]));
