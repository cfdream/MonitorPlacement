...
class GetNumTasksVsObjective():
    def get_result(self, raw_fname):
        ...
        for line in lines:
            ...
            max_node_flows = int(items[2])
            num_pairs = int(items[3])
            latency = int(items[4])
            flow_avg_latency = int(items[6])
            ...
            node_flow_num_vs_performance[max_node_flows].append(
                (num_nodes, num_tasks, num_pairs, latency, flow_avg_latency)
            )
        ...
        for max_node_flows, tuples in sorted(node_flow_num_vs_performance.items(), key=lambda item: item[0]):
            ...
            print(max_node_flows, avg_num_pairs, stdv_num_pairs, avg_latency, stdv_latency, avg_flow_avg_latency, stdv_flow_avg_latency) 
